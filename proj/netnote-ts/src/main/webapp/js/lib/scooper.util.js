...
function parseUrl(url) {
	...
	if (aidx < 0) aidx = url.length;
	if (qidx < 0) qidx = url.length;
	var ret = {
		'baseUrl': url.substring(0,Math.min(aidx,qidx)),
		'anchor': '',
		'params': {}
	};
	// 页内锚链接
	if (aidx < url.length) {
		ret.anchor = url.substring(aidx+1);
	}
	// 获取 query-string
	if (qidx < url.length) {
		var qs = url.substring(qidx+1,aidx);
		//
...
function SingleSelection(cfgs) {
	...
	this.getSelectionIndex = function() {
		return curSelectItem ? $(curSelectItem).index() : -1;
	};
	// e - HTMLElement
	this.onSelectionChange = function(e){};
	//

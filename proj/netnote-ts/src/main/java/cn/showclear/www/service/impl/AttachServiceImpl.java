...
public class AttachServiceImpl implements AttachService {
     ...
     */
    @Override
    public ReturnFile downloadAttach(int attachId, String username) throws Exception {
        ...
        try {
            ...
            if (shareFlag == CommonConstant.UNSHARE) {//附件所在的note没有被分享，需要校验用户名
                if (user == null || user.getId() != attach.getUserId()) return null;
...
//            CloseableHttpResponse downloadFile = client.execute(httpGet);
     ...
     * (在网盘未部署的时候，容易产生一些问题，比如在未与前端发生交互的情况下，写入数据库却没有存入网盘)
     *
     ...
     */
    @Override
    public void uploadAttach(ReturnFile returnFile, int noteId, String username) throws Exception {

        //检查笔记的所有权
        User user = userDao.selectUserByUsername(username);
        Note note = noteDao.selectNoteById(noteId);
        if (note == null || user == null || note.getUserId() != user.getId()) throw new Exception();
...
//            File file = new File(CommonConstant.netNoteTempLocal,username + File.separator + returnFile.getFileName());
            ...
            if (fileId != 0)
                attachDao.insertAttach(new Attach(returnFile.getFileName(), size, noteId, user.getId(), fileId));
     ...
     */
    @Override
    public void deleteAttachById(int attachId, String username) throws Exception {

        //校验附件的所有者
        Attach attach = attachDao.selectAttachById(attachId);
        User user = userDao.selectUserByUsername(username);
        if (user == null || attach == null || user.getId() != attach.getUserId()) return;
...
//            nvps.add(new BasicNameValuePair("fID", String.valueOf(attach.getNetDiskId() * 2)));
    ...
    private void getNotesByDirId(int directoryId, List<Note> notes) throws Exception {
        //加入目录下的每一个笔记
     ...
     * @return
     */
    private String checkTempLocal(String username) throws Exception {
